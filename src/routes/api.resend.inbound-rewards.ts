      ...
      POST: async ({ request }) => {
        ...
        if (attachments.length === 0) {
          ...
          const portal = await tryPortalEmailImport({
            to: env.to as string | string[] | undefined,
            subject: typeof env.subject === "string" ? env.subject : null,
            metaAttachments,
            emailId:
              (env.email_id as string | undefined) ?? (env.id as string | undefined) ?? null,
          });
          if (portal.handled) return jsonResp(200, portal.body ?? { lane: "portal_import" });
