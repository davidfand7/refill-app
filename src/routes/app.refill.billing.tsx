/**
 * /app/refill/billing — pricing plan + monthly invoices (v366 — killshot).
 *
 * Where the $$ flows. Three plans, one click to choose.
 *
 * v366 ships invoices in 'draft' status (no Stripe push). v366.x adds
 * the payment method capture + Stripe flow when a real spa explicitly
 * opts in. For now: the spa sees the math, audits the receipts,
 * commits to the plan that fits their model.
 */
...
// v411.2 — wrapper that gates the legacy Emma(OS) billing page on shell.
//
// /app/refill/billing is the LEGACY Emma(OS) demo billing surface on
// emma.agentiport.com. On the Refill host (app.getrefill.app), the
// canonical billing surface is /app/billing (the v391 Refill production
// page with real Stripe). Karen-style tenants on Refill should NEVER see
// the Emma-branded copy on this page — even if they typed the URL,
// followed a stale bookmark, or arrived via a search result.
//
// The redirect lives inside this wrapper (not in app.tsx
// REFILL_ALLOWED_PREFIXES) so it's path-specific and survives any
// future allowlist refactor. The wrapper is a separate component (not
// an early-return inside BillingPage) to honor React's rules of hooks
// — BillingPage's many useState/useCallback/useEffect calls would
// violate rules-of-hooks if the conditional return preceded them.
...
> = {
  performance: {
    label: "Performance",
    tagline: "Free. You only pay when Emma recovers revenue.",
    icon: Zap,
    tone: "primary",
  },
  predictable: {
      ...
      <div className="px-6 lg:px-10 py-8 max-w-5xl space-y-8">
        ...
        {!active && !loadError && (
          <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-5">
            ...
            <p className="text-sm text-ink-soft">
              Pick a plan below. Until you do, Emma's recovery engine still
        ...
        <div className="rounded-xl bg-muted/30 border border-border p-4 flex items-start gap-3">
          ...
          <p className="text-xs text-ink-soft leading-relaxed">
            ...
            consider every invoice a receipt — Emma is showing you what she
            would have charged.
          </p>
        </div>
      </div>
