/**
 * /app/refill/agents/rescue — the Refill agent operator surface (v1.34.6).
 *
 * Mirrors /app/refill/agents/preshow's operator-transparency framing but
 * Refill stays single-policy per spa (no Refill PROFILES). Karen controls:
...
function RescueAgentPage() {
  ...
  const simulate = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(
          `Refill fired on a ${res.slotDurationMin}-min slot — ${res.offersSent} offer${res.offersSent === 1 ? "" : "s"} sent to the proxy inbox. (test slot ${res.testAppointmentId.slice(0, 8)})`,
        );
      } else {
        toast.warning(`Refill didn't send: ${res.reason ?? "unknown"}`);
      }
    } catch (e) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      <PageHeader
        ...
        breadcrumbs={[
          { label: "Recapture", to: "/app/refill/recovery" },
          { label: "Refill" },
        ]}
        actions={
          isAdminView ? (
      ...
      <div className="flex-1 px-4 py-6 lg:px-10 max-w-[960px] w-full mx-auto space-y-5">
        {loadError ? (
          <div className="rounded-2xl border border-rose/30 bg-rose-soft p-5 text-sm">
            <div className="font-semibold text-rose">Couldn't load Refill settings</div>
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold text-ink">
                  {policy.enabled ? "Refill is on" : "Refill is paused"}
                </div>
                <p className="text-xs text-ink-soft mt-0.5">
                  {policy.enabled
                    ? "When an appointment cancels, Refill texts your waitlist with a tap-to-claim offer for the open slot."
                    : "Cancellations won't be re-offered until you turn Refill back on."}
                </p>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {policy.eligibleTreatments.length === 0 && (
                    <span className="text-[11px] text-ink-faint italic">
                      No restrictions — Refill fires for every cancelled
            ...
            <section className="rounded-xl border border-dashed border-rule bg-paper/50 px-5 py-4 text-[12px] text-ink-soft flex gap-3">
              ...
              <div>
                <strong className="text-ink">How Refill works</strong> — when an
