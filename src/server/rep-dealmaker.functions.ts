...
const composeInput = z.object({
  ...
  quarterlySpendUsd: z.number().nonnegative().max(100_000_000).nullable().optional(),
  annualSpendUsd: z.number().nonnegative().max(100_000_000).nullable().optional(),
...
export const composeRepProposalFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<RepProposalOutput> => {
    ...
    return composeRepProposal({
      ...
      quarterlySpendUsd: data.quarterlySpendUsd ?? null,
