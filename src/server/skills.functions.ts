...
//
// Autonomous Refill is EARNED, not default: the owner unlocks it after approving
    ...
    if (!isViewingAs) {
      ...
      if (tpl.materializer === "autonomous") {
        ...
        if (!earned) {
          ...
          throw new Error(
            `Autonomous Refill unlocks after you've approved ${AUTONOMY_RUNG_APPROVALS} held offers — you've approved ${have} so far. Keep reviewing the queue and it'll open up.`,
          );
        }
      }
    }
...
//
// Feeds the "Autonomous Refill" catalog card: how many held offers the owner has
